...
public class E08_File {
	 ...
	 * # 파일 시스템
	 * 
	 * - 하드 디스크에 저장된 데이터들을 손쉽게 관리할 수 있도록 설정한 일부 구역
	 * - 파일 시스템의 내용을 디렉토리 구조로 볼 수 있게 하는것이 탐색기
	 */
	public static void main(String[] args) {
		//윈도우즈에서 경로는 대소문자를 구분하지 않는다
		File f1 = new File("Files/test.txt");
		File f2 = new File("Files/");
		File f3 = new File("C:/java_PJJ/");
		...
		// isAbsolute() : 해당경로가 절대 경로인지 여부를 검사
		 ...
		 *  		-현재 위치가 JavaFullStack 폴더인 경우에만 올바르게 작동
		 */
		...
		System.out.println("해당 파일이 있나요? " + new File("./logs/").exists());
		
		//mkdir() : 해당 경로에 디렉토리를 생성
