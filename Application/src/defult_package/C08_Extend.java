...
public class C08_Extend {
	...
	public static void main(String[] args) {
		...
		Coffee c2 = new Coffee(3,3500,"찐한 커피");
		
		//부모 클래스인 커피 클래스의 변수와 메서드를 그대로 물려받아 사용중
		c1.shot = 3;
		c2.shot = 4;
...
class Coffee {
	...
	// 생성자를 하나도 정의하지 않았을 때에는 아무것도 없는 기본 생성자를 제공하지만
...
class Americano extends Coffee{
	...
	//부모 클래스에 이미 존재하는 기능을 마음대로 덮어써서 사용하기(오버라이드, override)
	void drink() {
		System.out.println("아메리카노는 물을 타서 밍밍합니다");
	}
	// 메서드가 오버라이드 되었더라도 부모 클래스의 메서드를 불러다 사용하는것이 가능하다
	void superDrink() {
		super.drink();
	}
	...
	void check() {
		if(water < 100) {
			System.out.println("현재 상태는 매우 진한 아메리카노입니다");
		}else if(water<200) {
			System.out.println("현재 상태는 약간 진한 아메리카노입니다");
		}else if(water < 300) {
			System.out.println("현재 상태는 평범한 아메리카노입니다");
		}else if(water < 400) {
			System.out.println("현재 상태는 약간 연한 아메리카노입니다");
		}else if(water < 500) {
			System.out.println("현재 상태는 연한 아메리카노입니다");
		}
	}
}