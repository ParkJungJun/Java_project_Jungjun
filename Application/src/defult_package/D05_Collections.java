...
public class D05_Collections {
	...
	public static void main(String[] args) {
		...
		//Arrays.compare
		// : 두 배열을 부호와 상관없이 비교하여 결과를 알려준다
		//   어떤 메서드에 unsigned가 붙어있으면 부호를 무시하는 메서드이다
//		System.out.println("unsigned: " +
//		Arrays.compareUnsigned(stu1, stu3));
		// Arrays.sort(arr) : 전달한 배열을 크기순으로 정렬해준다
		Arrays.sort(stu3);
		System.out.println(Arrays.toString(stu3));
		
		//Collections.addAll(Collection, values...) : 값을 한번에 더한다
		List<Integer> nums = new ArrayList<>();
		Collections.addAll(nums, 99,20,-99,40,100,60,200,80);
		...
		// dset의 크기가 src보다 작다면 에러가 발생할 수 있다.
		...
		// : 해당 값이 몇 번 나오는지 세어준다
		int howMany = Collections.frequency(snacks, "프링글스");
