...
public class D01_ArrayList {
	 ...
	 * - 상황마다 데이터를 가장 효율적을 다룰 수 있는 방법에 대해 연구해놓은
	...
	public static void main(String[] args) {
		...
		//list.get(index) : 리스트의 해당 번째 요소를 꺼내서 반환해주는 메서드
		...
		//add(index, item) : 요소를 원하는 위치에 추가한다
		countries.add(2, "프랑스");
		System.out.println(countries);
		// countries(item) : 해당 요소가 리스트에 포함되어 있는지 여부를 리턴해준다
		System.out.println(countries.contains("영국"));
		System.out.println(countries.contains("한국"));
		System.out.println(countries.contains("일본"));
		System.out.println(countries.contains("프랑스"));
		
		if(countries.contains("독일")) {
			System.out.println("있습니다");
		}else {
			System.out.println("없습니다");
		}
		// remove(index) : 해당 번째의 요소를 지운다
						 //방금 리스트로 제거한 요소의 값을 반환한다
		// remove(item) : 해당 값을 지닌 요소를 하나 지운다
		String removed = countries.remove(5);
		System.out.println("지워진 것은 " + removed);
		System.out.println(countries);
		
		removed = countries.remove(countries.size() - 1);
		
		// addAll(Collection) :다른 컬랙션을 전달하면 모두 더해준다
		
		/*
		 * # Java Collection Interface
		 * 
		 * - 자바에서 제공하는 자료구조 인터페이스
		 * - Collection은 언제든지 다른 컬렉셔능로 쉽게 변환이 가능하다
		...
		//값을 전당해 해당 값을 지울때 삭제 성공여부가 반환
		boolean delete = countries.remove("일본");
		System.out.println("삭제 성공 여부 " + delete);
		System.out.println(countries);
		// set(index,item): 해당 번째의 요소를 수정한다
		countries.set(3,"사과");
		System.out.println(countries);
		// sublist(start, end) :리스트를 원하는만큼 잘라서 반환한다
		List<String> result = countries.subList(2,5);
		System.out.println(result);
		
		//리스트의 업캐스팅
		// 실제 구현은 다르지만 우리는 신경 안쓰고 똑같은 메서드로 두 클래스를 다룰 수 았음
		List<String> l1 = new ArrayList<>();
		List<String> l2 = new LinkedList<>();
		
		l1.add("AA");
		l2.add("ABC");
//		ArrayList l3 = new ArrayList(100);
//		System.out.println(l3.size());
//		for(int i = 0; i < l3.size(); ++i) {
//			l3.get(i) = (int)(Math * 1000 + 1000);
//		}
	}
