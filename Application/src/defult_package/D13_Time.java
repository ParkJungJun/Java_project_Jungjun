...
public class D13_Time {
	 ...
	 * - Calendar가 set(), add()를 통해 원본 인스턴스를 반환하는 것이
	...
	public static void main(String[] args) {
		...
		LocalDate todayEtc = LocalDate.now(ZoneId.of("America/Cuiaba"));
		
		LocalTime thisTime = LocalTime.now();
		LocalTime thisTimeEtc = LocalTime.now(ZoneId.of("America/Cuiaba"));
		
		// ZoneId.getAvailableZoneIds() : 사용가능한 ZoneId를 Set으로 사용한다
		...
		System.out.println(LocalDateTime.of(2002, 7, 16, 22, 59, 59, 999_999));
		
		// plusXXX() : 원하는 것을 원하는 만큼 더한 새 인스턴스를 반환
		...
		System.out.println("2천년 전: " + today.minusYears(2000));
		...
		// dow.getDisplayName(TextStyle, Locate)
		// - TextStyle : 길게 OR 짧게
		// - Locale : 언어 설정
		System.out.println("요일 이름: " +
				dow.getDisplayName(TextStyle.SHORT, Locale.KOREAN));
		
		//Month 클래스
		Month mon = today.getMonth();
		System.out.println(mon.getValue());
		System.out.println(mon.getDisplayName(TextStyle.FULL, Locale.KOREAN));
