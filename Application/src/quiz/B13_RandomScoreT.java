...
public class B13_RandomScoreT {
	...
	public static void main(String[] args) {
		...
		double avg = Math.round(sum / (double) scores.length * 100)/ 100.0;
		
		System.out.println("총점 " + sum);
		System.out.println("평균 " + sum / (double) scores.length);
		
		// 첫 번째 점수로 두 값을 초기화 한다
		int best = scores[0];
		int worst = scores[0];
		// 두 번째 점수부터 체크를 시작 한다
		for(int i = 0; i <scores.length; ++i) {
			// 현재 best에 들어있는 값보다 더 큰 함수가 있다면 best를 교체한다
			if(best < scores[i]) {
				best = scores[i];
			}
			
			// 현재 worst에 들어있는 값보다 더 작은 정수가 있다면 worst를 교체한다
			if(worst > scores[i]) {
				worst = scores[i];
			}
		}
