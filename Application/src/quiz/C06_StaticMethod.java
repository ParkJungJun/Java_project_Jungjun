...
public class C06_StaticMethod {
	/*
	 	...
	 	# 인스턴스 메서드
	 	- 각 인스턴스 메서드 마다 다른 결과를 내는 메서드
	 	- 각 인스턴스 변수를 활용하기 때문에
	 	- new를 통해 인스턴스가 생성된 이후부터 활용할 수 있다.
	 	
	 	# 스태틱 변수
	 	- 모든 인스턴스에서 함께 사용하는 변수
	 	- 한 클래스에 하나 밖에 없다.(클래스 변수)
	 	- 해당 클래스에 인스턴스가 하나도 없는 상황에서 사용할 수 있다.
	...
	public static void main(String[] args) {
		...
		// 인스턴스가 수천개 있어도 static 자원은 하나이기 떄문에
		// 클래스 이름에 .을 찍고 접근하는 것이 올바른 방식이다.
		System.out.println("카드 너비 : "+ c1.width);
		Card c3 = Card.getRandomCard();
		System.out.println(c3.shape+" "+c3.number);
